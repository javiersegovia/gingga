...
export function getVercelToolset({ entityId }: { entityId?: string | null }): VercelAIToolSet {
  try {
    return new VercelAIToolSet({ apiKey: webEnv.COMPOSIO_API_KEY, entityId: entityId ?? undefined })
  }
  catch (_error) {
...
> {
  return {
    googlesheets: {
      ...
      integrationId: webEnv.COMPOSIO_GOOGLESHEETS_INTEGRATION_ID,
      image: '',
    },
    googlecalendar: {
      ...
      integrationId: webEnv.COMPOSIO_GOOGLECALENDAR_INTEGRATION_ID,
      image: '',
    },
    gmail: {
      label: 'Gmail',
      ...
      integrationId: webEnv.COMPOSIO_GMAIL_INTEGRATION_ID,
      image: '',
    },
    googledocs: {
      label: 'Google Docs',
      ...
      integrationId: webEnv.COMPOSIO_GOOGLEDOCS_INTEGRATION_ID,
      image: '',
    },
  }
}
