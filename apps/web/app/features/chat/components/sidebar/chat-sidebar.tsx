...
import type { Chat, User } from '@/db/types'
...
function AuthenticatedHistoryList() {
  const { data: chats } = useUserChatsSuspenseQuery()
  ...
  return (
    <SidebarMenu className="gap-0">
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        {chats.map((chat) => {
          const chatLink = chat.agentId
            ? '/chat/agent/$agentId/chat/$chatId'
            : '/chat/$chatId'
          const linkParams = chat.agentId
            ? { agentId: chat.agentId, chatId: chat.id }
            : { chatId: chat.id }
